...
$query_rs_content = $connexion->prepare("SELECT * FROM adminusers ORDER BY id_admin ASC");
$query_rs_content->execute();
$row_rs_content = $query_rs_content->fetch();


$tg = $row_rs_content['photo'];
                       ...
                       <!--/tabs-inner-->
                        <div class="tab-inner">
                            ...
                            <?php if((!isset($_GET['id']))&&(!isset($_GET['ajout']))){ ?>
                              ...
                              $request = $connexion->query("SELECT COUNT(id_admin) FROM adminusers");
                              $count = $request->fetchColumn();
                                ?>
                                 <?php if($count < 3){ ?>
                                      <h2 class="inner-tittle">Zone admin<a href="adminusers.php?ajout" class="btn btn-warning"><i class="fa fa-plus"></i> Ajouter</a></h2>

                                      <?php
                                   }else
                                      {
                                        ?>
                                        <h2 class="inner-tittle">Zone admin</h2>
                                        <?php
                                        $Session9 = new Session();
                                        $Session9->setFlash("Le nombre de compte administrateur autorisé est atteint(3 maximum)",'info');
                                        $Session9->flash_info();
                                      } ?>
                              ...
                              <div class="graph-form">
                                  ...
                                  <tr>
                                    <td><a href="adminusers.php?id=<?php echo $row_rs_content['id_admin']; ?>"style="color: green;"><i class="fa fa-edit"></i>Modif.</a>  |
                                    <?php
                                    /*nombre d'administrateur */
                                      $request = $connexion->query("SELECT COUNT(id_admin) FROM adminusers");
                                      $count = $request->fetchColumn();
                                    ...
                                    <a href="adminusers.php?supprimer=<?php echo $row_rs_content['id_admin']; ?>"style="color: red;"><i class="fa fa-trash-o"></i>Suppr.</a>
                                  <?php } ?>
                                  </td>
                                  ...
                                  <?php
                                  $usersphoto = $connexion->prepare("SELECT * FROM adminusers WHERE photo = '".$row_rs_content['photo']."'");
                                  $usersphoto->execute();
                                ...
                                <?php } ?>
                                  ...
                                  <?php
                                    if(isset($_GET['supprimer'])){
                                      ...
                                      $req = $connexion->prepare("DELETE FROM adminusers WHERE id_admin=?");
                                      $req->execute(array($_GET['supprimer']));
                                      $Session12 = new Session();
                                      $Session12->setFlash("Vous venez de supprimer un administrateur",'error');
                                      $Session12->flash_danger();
                                      $insertGoTo = "adminusers.php?OK";
                                  ...
                                  <?php if(isset($_GET['id'])){ ?>
                                   <h3 class="inner-tittle">Modifier un compte Administrateur</h3>
                                    <div class="graph-form">
                                                ...
                                                if(strlen($_POST['password']) > 8){
                                                  if($row_rs_utiles['photo'] != null){
                                                    if($_POST['couleur'] != $row_rs_utiles['couleur']){
                                                      $updatephoto = $connexion->prepare("UPDATE adminusers SET photo = null WHERE id_admin='".$_GET['id']."'");
                                                      $updatephoto->execute();
                                                    }
                                                  }
                                                    ...
                                                    if( in_array($videoFileType,$extensions_arr) ){
                                                      ...
                                                      if(($_FILES['fichier']['size'] >= $maxsize || $_FILES["fichier"]["size"] == 0)) {
                                                        ...
                                                        if(move_uploaded_file($_FILES['fichier']['tmp_name'],$target_file)){
                                                          $updateSQL = $connexion->prepare("UPDATE adminusers SET nom=:nom, prenom=:prenom, tel=:tel, email=:email, passwords=:pass_hash, photo=:photo WHERE id_admin='".$_GET['id']."'");
                                                      ...
                                                      $updateSQL = $connexion->prepare("UPDATE adminusers SET nom=:nom, prenom=:prenom, tel=:tel, email=:email, passwords=:pass_hash, couleur=:couleur WHERE id_admin='".$_GET['id']."'");
                                                      if($_POST['password'] != $row_rs_utiles['passwords']){
                                                      $pass_hash = password_hash($_POST['password'], PASSWORD_DEFAULT); //Hash mdp
                                                      }else{
                                                        $pass_hash = $_POST['password'];
                          ...
                          }else
                          {
                            ...
                            $Session3->flash_danger();
                                     ...
                                     <a href="adminusers.php" class="btn btn-warning">Retour à la liste</a>

                                  <?php } ?>

                                  <?php if(isset($_GET['ajout'])){ ?>
                                  <h3 class="inner-tittle">Ajouter un administrateur</h3>
                                    <div class="graph-form">
                                                        ...
                                                        if( !empty($_FILES['fichier']['name']) )
                                                        {
                                                          ...
                                                          if( in_array($videoFileType,$extensions_arr) ){
                                                            ...
                                                            if(($_FILES['fichier']['size'] >= $maxsize || $_FILES["fichier"]["size"] == 0)) {
                                                              ...
                                                              // Upload
                                                              if(move_uploaded_file($_FILES['fichier']['tmp_name'],$target_file)){
                                                                $insertSQL = $connexion->prepare("INSERT INTO `adminusers`(`id_admin`, `nom`, `prenom`, `tel`, `email`, `passwords`, `photo`) VALUES (:id, :nom, :prenom, :tel, :email, :pass_hash, :photo)");
                                                            ...
                                                            $insertSQL = $connexion->prepare("INSERT INTO `adminusers`(`id_admin`, `nom`, `prenom`, `tel`, `email`, `passwords`,`couleur`) VALUES (:id, :nom, :prenom, :tel, :email, :pass_hash, :couleur)");
