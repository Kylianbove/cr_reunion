...
(function($) {
	$.fn.extend({
		...
		if (options.weekStart == 'Sun') {
			...
			$('#' + uniqueId).append('<div class="monthly-day-title-wrap"><div>Lun</div><div>Mar</div><div>Mer</div><div>Jeu</div><div>Ven</div><div>Sam</div><div>Sun</div></div><div class="monthly-day-wrap"></div>');
		} else {
					...
					$(d).find('event').each(function(){
						...
						function updateList(){
							if (fullendDate && options.eventList == true) {
								$('#' + uniqueId + ' .monthly-event-list').append('<a '+eventLink+'  data-eventid="'+ eventId +'"><div class="monthly-list-date">'+startArr[2]+' '+monthNames[startArr[1] - 1]+' '+startArr[0]+' - '+endArr[2]+' '+monthNames[endArr[1] - 1]+' '+endArr[0]+'</div><div class="monthly-event-indicator" style="background:'+eventColor+'"></div> '+eventTitle+'</a>');
							
							} else if(options.eventList == true) {
								$('#' + uniqueId + ' .monthly-event-list').append('<a '+eventLink+'  data-eventid="'+ eventId +'"><div class="monthly-list-date">'+startArr[2]+' '+monthNames[startArr[1] - 1]+' '+startArr[0]+'</div><div class="monthly-event-indicator" style="background:'+eventColor+'"></div> '+eventTitle+'</a>');
							}
						}
